...
namespace PaletteMaker
{
    class NesPalette
    {
        ...
        public void SelectPalette(int index)
        {
            ...
            {
                case 1: // PAL
                    Array.Copy(Palette_PAL, RawPalette, 1536);
                    break;
                case 2: // RGB
                    Array.Copy(Palette_PC10, RawPalette, 1536);
                    break;
                case 3: // VS 1
                    Array.Copy(Palette_VS_0001, RawPalette, 1536);
                    break;
                case 4: // VS 2
                    Array.Copy(Palette_VS_0002, RawPalette, 1536);
                    break;
                case 5: //  VS 3
                    Array.Copy(Palette_VS_0003, RawPalette, 1536);
                    break;
                case 6: //  VS 3
                    Array.Copy(Palette_VS_0004, RawPalette, 1536);
                    break;
            }
        }
        ...
        int	getPhase(double[] wave)
        {
	        ...
	        for (k = 0; k < 3; k++)
	        {
		        ...
		        for (j = 0; j < 12; j++)
		        {
			        error[j] = 0;
			        for (i = 0; i < 12; i++)
				        error[j] += Math.Abs((amp * Math.Sin((i * 30 + j * segsize + angle) * Math.PI / 180.0) + offset) - wave[i]);
			        curerror += error[j];
		        }
		        theBase = 0;
		        for (j = 0; j < 12; j++)
        ...
        public void GenerateNTSC(int hue, int sat)
        {
            RawPalette = new byte[8, 64, 3];

	        double black = 0.519;
	        double white = 1.443;
	        double[,] voltage = new double[2, 4] {
		        {1.094,1.506,1.962,1.962},
		        {0.350,0.519,0.962,1.550}
	        };

	        byte[,] phases = new byte[12, 12] {
			        ...
			        for (z = 0; z < 16; z++)
			        {
				        ...
				        for (i = 0; i < 12; i++)
				        {
					        if (z == 0)
						        ...
						        wave[i] = (phases[z-1, i] == 1) ? voltage[0, y] : voltage[1, y];
					        else if (z == 13)
						        wave[i] = voltage[1, y];
					        else	wave[i] = black;
					        if ((emphasis[x, i] == 1) && (z < 14))
						        wave[i] = wave[i] * 0.75;
				        }
				        ...
				        S = Math.Sqrt(S / 12.0) * sat / 50.0;

				        H = Math.PI * (270 + getPhase(wave) + hue) / 180.0;
				        ...
				        RawPalette[x, (y << 4) | z, 0] = (byte)Math.Max(0, Math.Min(255, R));
				        RawPalette[x, (y << 4) | z, 1] = (byte)Math.Max(0, Math.Min(255, G));
                        RawPalette[x, (y << 4) | z, 2] = (byte)Math.Max(0, Math.Min(255, B));
			        }
		        }
	        }
        }
